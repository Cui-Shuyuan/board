...
> 2026-09-21 用户进一步要求：**新建动画和修改动画**都必须按同一条固定顺序走 ——
> **改脚本（文字版） → 调 BoardAI API 确认合法性 → 套原语 → 对账**。
> 禁止一上来先改 events/Unity 再回来补文字；否则契约和引擎会一起自洽，现有检查全部抓不到语义错误。
> 同时：**动画职责越少越好**，`events` 只是脚本的执行层；脚本和动画必须永远一一对应，
> 不能出现“文字说了、结构没做”或“结构做了、文字没提”。
> 操作细则已写进 `games/splendor/tutorial/anim/README.md` 的「新建 / 修改一条 cue 的标准动作」。
