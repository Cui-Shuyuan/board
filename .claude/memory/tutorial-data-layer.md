...
- **L1 口播脚本层**：播放单元树、narration、source_refs、quick/full 版本标记。叶子按 1～3 句口播切，不按规则小节切。已产出 `games/splendor/tutorial/full.lrc`（LRC-like，59 cues），由 `scripts/validate_timed_script.py` 校验/解析。
- **TTS 生成**：`scripts/tts_doubao.py` 使用豆包语音合成 2.0 WebSocket 双向流式接口，按 cue 输出 `mp3 + subtitle.json`；前 3 条已验证，全量后用 `--write-lrc` 生成 `full.tts.lrc`。
