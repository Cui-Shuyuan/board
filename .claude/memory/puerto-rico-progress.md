...
- PR 建筑费用建模：`building` 只声明 `"price": N` 作为接口；`buy_building`（`<ontology::play>`）里定义 `<ontology::cost>` + `<ontology::instant_cost>` + `<ontology::transfer>`，quantity 引用 `this.<ontology::piece>.price`（再按采石场折扣/选角者特权减免）。
