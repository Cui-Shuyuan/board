...
- **终局计分轨职责统一（2026-08-11，用户指出）**：`final_scoring_area_hex` 的 11 个 slot 各挂 `<ontology::effect>`（9 类别格=各自计分规则「每有 X 得本格下方印的分值/板块分数」、stage_partition=阶段分区结算、point_bonus=point bonus 结算）；**slot effect 写成完整 trigger 形式（用户要求）——condition 统一模板「`<phase_indicator>` 位于本格（`<final_scoring_area_hex>.<格名>`）」，content 每格自定义**（marker 位于该格即触发该格计分，结构化非 description）；`final_scoring` 简化为两步骤：`final_scoring_categories`（round，loop.until **final_scoring_track_end=位于 point_bonus（第 11 格走完全轨）**，evaluate score_next_category「推进到下一格——该格 slot 效果自动结算」）+ `determine_winner`（evaluate）——**两个 trigger 步骤删除**（职责移交轨 slot）。**教训**：批量脚本 out 构建漏掉 `lines[:slots_i]` 导致 concepts.json 被截断 3000+ 行（git checkout 恢复，无未提交修改丢失）——批量脚本先 commit + 先验证输出构建完整性
