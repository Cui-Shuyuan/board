...
- **终局计分二三轮改 trigger（2026-08-10，用户指出）**：`final_scoring_stage_partition` / `final_scoring_point_bonus_cards` 原 specifies `<ontology::phase>` 错误——它们是计分结算操作不是阶段；改为 trigger（condition=轨道位置「`<phase_indicator>` 推进到 `<final_scoring_area_hex>.<stage_partition>`/`.<point_bonus>`」，content=instant_content 描述 per_player 计分），参照 event_era_scoring 模式。**术语约定（用户纠正）**：终局计分没有「第几轮」概念——就是一个 pipeline 沿轨逐格推进结算（9 类别格→控制台 A→控制台 B），「轮」术语已从终局计分区域全部清除（condition 全部用轨道位置表达）。终局计分四步类型：categories=round（循环逐格）、stage_partition=trigger、point_bonus=trigger、winner=evaluate
