...
### 追加（用户 2026-09-21）：宝石设置按 2 人局实际动作演，不做 4→5→7→4

- UI18=`setup.gems.003.1` 一次 `create` 每色 **4 枚**；这是 2 人局实际一开始就会做的动作。
- UI19（口播 3 人 5 枚）、UI20（口播 4 人 7 枚）**画面保持 4 枚**，不再累计加到 5/7；
  口播只说明其他人数，不能演成“先全部拿出来摆好、后面再放回去”。
- UI21 确认供应堆；UI22 只放 5 枚黄金，**不再执行 7→4 的 destroy**。
- `validate_anim_rules.py` 的 `FROZEN_FROM` 提前到 `setup.gems.003.1`：2 人局 4 枚一旦建立就锁住。
- 对账 0 处不一致，9 条取景警告。
