...
2. **`order` / `slot` 还没有本体说法**：`ZoneItem` 的顺位是状态的一部分，
   但本体 `<zone>` 上没有"有序表"这种字段。现在只在引擎内部用，契约断不了"第几位"。
3. **`create` / `stack` 的 `destination` 仍是实现层**：牌堆"该 create 还是 transfer"没定论。
4. **动画数据里的 zone id 还是本作专用名**（`gem_supply_diamond` 等）→ 跨游戏复用还不成立。
   下一步若要复用，zone 的选择应改成"概念 + 属性"（像 `what` 那样），id 只留在 stage 里。
5. 其余 92 条 cue 还没动画（每条的契约也还没写）。
