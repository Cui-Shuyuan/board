...
**相机：俯角是可调参数，当前 90°（正俯视）★（2026-09 用户改）**——原定 50° 俯角固定机位（与客人坐在桌边的视角一致、带入感最好；正交相机 + 微倾 billboard 保证可读性）。但 2026-09 用户看到实物后要求「去掉 50° 摄像机这个设定，恢复所有组件的原始尺寸」：正交相机没有透视，50° 只带来**高度被 cos50°≈0.64 压扁**、没有纵深，观感上就是「组件被压矮了」。现在 `stage.board.camera_pitch` = 90（正俯视，地面 1:1 映射到画面），并且**所有组件面片跟着相机转**（`SpriteRotation`），所以 pitch 改成任何值都只改变布局的纵深压缩、不会再压扁组件本身——想找回代入感，改这一个数即可。
...
- 动画继续只用固定的那组原语（当前 12 个，见 [[tutorial-data-layer]]），LLM 只填参数，不写新协程；不做 authoring 界面。
- 试点《璀璨宝石》，标杆稿为 `doc/splendor/口播稿.md`（full 版）；已拆出 `games/splendor/tutorial/full.lrc`（109 cues），full TTS 已全量生成并编译 `full.runtime.json`；Unity 纯音频 cue 播放器 v0 已写入 client。
