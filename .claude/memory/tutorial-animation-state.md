...
## 【工具缺陷】离屏出图在"朝向"上不可信（2026-09，已两次）

`DumpState` 的数据明确说三摞牌堆 `显示卡背=36/26/16`（正确），
但同一份代码的 `CaptureTimeline` 出图里牌堆显示的是**卡面**。
两次出现同类矛盾（另一次是"读到了上一次出图的残留文件"）。

**结论：朝向类问题以实机（用户的编辑器）为准，不依赖离屏出图。**
若要用出图判断朝向，必须先修好这个工具 —— 否则会像这次一样白查。
