...
`SelfTestOrientationMatchesSprite`：逐帧检查
- 牌堆的牌必须显示**卡背**（有独立背图用背图，没有则显示它自己的 FaceSprite —— 那张就是卡背扫描图）
- **已落位**的市场牌必须显示**真卡面**（正在飞的那张由片段控制，先不判）

这两个问题它都抓得到 —— 说明"朝向"这类**渲染层不变量**必须单独测，
契约的"张数统计"是看不见它的。
...
| 字段 | 含义 |
|---|---|
| `ZoneItem.Flipped == true` | **已翻到正面** → `RefreshFace` 显示 `BackSprite` |
| `ZoneItem.Flipped == false` | 未翻开 → 显示 `FaceSprite` |

对卡牌而言 `FaceSprite` 是**卡面**、`BackSprite` 是**卡背**，
而 `Flipped` 记的其实是"要不要显示卡背"。名字别扭，但改动会牵连很多地方。

**教训**：我在这一轮里把测试判据写反过一次（以为"朝上=FaceSprite"），
结果工具报了一个不存在的 bug。**判据必须对着引擎的实际约定写，不能凭直觉。**
