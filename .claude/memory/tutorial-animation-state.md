...
## 【已修】"从牌堆底发牌" = PickFront 取反了（2026-09）

用 `DumpPileOrder` 打印每张牌的坐标，一眼看出：

```
order= 0  pos=(-2.862,-0.508)  最左   ← 与其他牌重合的那一叠
order= 9  pos=(-2.862,-0.508)  最左
order=39  pos=(-2.750,-0.620)  最右   ← 错开最多 = **画在最上面那张**
```

当前位置公式 `lift = min(capacity-1-order, 7)` 让 **order 最大的一张错开最多**，
而 `PickFront` 取的是 **order 最小** —— 那正好是**埋在重合块最里面**的那张。

**两者必须一致**：位置公式把"最上面"画在 order 最大那端，`PickFront` 就必须取 order 最大。

改成 `item.Order > best.Order` 后：

```
t=3.5  40张 台8     ← 发牌前
t=3.6  39张 台8     ← 发第 1 张，台阶不变
t=4.8  36张 台8     ← 发 4 张，台阶仍不变
t=5.2  deck2 30→29   ← 三摞都在正确发牌
```

### 教训（这次最该记住的）

**"哪张会被发走"和"那张画在哪里"必须一起验证。**
我一直在单独调位置公式，从没把 `PickFront` 的选择和 `ZoneStore` 的绘制**对照**过。
`DumpPileOrder` 把两者打在同一份输出里，一眼就看出矛盾 —— 这个工具本该第一天就有。

### 关键工具

| 工具 | 回答什么 |
|---|---|
| `DumpPileOrder` | **哪张会被发走**、它画在画面哪里（order↔坐标↔方位） |
| `TraceState` | 逐帧状态（件数/正面数/台阶数），看动画过程中发生了什么 |
