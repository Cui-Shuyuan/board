...
## 契约的完备性：没写就是不该有（2026-09-19 用户定的原则）

> 该有的有，不该有的就没有。**脚本里没写有的，那就是没有。**
> 脚本里的和采样采的应该能明显对比出差别（一个有盒面、一个没有），
> 有这个信息就足够发现问题。

落成两条**可执行的检查**（都不是"记个笔记"）：

1. **`picture` 默认比对**（`check_cue_script.py`）：契约里没写 `picture` 就等于
   "这一帧不该有整幅图"。以前只在契约写了才比 —— 那样"忘了写"会静默跳过这一维，
   正是 cue 10 盒面漏掉的原因。
2. **契约必须覆盖事件碰过的 zone**（`validate_cue_anim.py`，静态查、不需要采样）：
   凡是 `transfer` / `create` / `destroy` / `stack` 碰过的 zone，契约的 `enter`/`exit`
   里必须有它（空也要写 `count: 0`）；有 `showbox` 就必须声明 `picture`。
   原理：**不声明 = 不在被比的集合里 = 永远发现不了**（比较逻辑没错也可能什么都查不出）。

两条都用金丝雀验证过：删掉 cue 12 的 `deck_level_1` 声明 → 立刻报；
删掉 `bg.intro.001.1` 的 `picture` → 立刻报；恢复后 0 警告。
**当前 17 条契约 0 警告**（各自事件碰过的 zone 都声明齐了）。

教训一句话：**漏检的根因常常不是"比较写错了"，而是"这一维根本不在被比的集合里"。**
所以每加一维状态，都要同时做三件事 —— 引擎跟踪它、采样导出它、契约能断言它（且默认"没写就是没有"）。

## 第三、四条检查（2026-09，宝石介绍那一轮补的）

3. **取景 ∩ 有内容的组件 vs 契约声明**（`check_cue_script.py`）：状态对账查不出
   "**不该出现在画面里**"的东西。取景框按引擎 `FitCamera` 的几何算出来，
   框里每一件如果契约没提到它的 zone 就报警告（用户定的措辞就是 warn ——
   入镜不一定是错的，但得有人说出来）。详见 [[tutorial-animation-state]] 对应小节。

4. **`face` 只对有背图的件成立**（`check_cue_script.py`）：契约给单面件
   （`sample_card_*` / `sample_back_*` / `blank_card_*` / `noble`）断 `face_up/face_down/face`
   时，检查器直接说"这一维对它不存在，去掉这条断言"。理由见下。

### 为什么"样本"和"真件"必须分开算（用户 2026-09 定）

宝石介绍不再从盒子里飞进来，而是**在展示位上凭空出现**；介绍完立即销毁、镜头切回供应区。
落地成两个东西：

- **展示位 zone**（`gem_display` / `gold_display`）：`concept: null`、`contains: []` ——
  它不是本体里的任何区域，只是"把宝石摆出来看"的地方，离供应堆很远。
- **样本模板**（`gem_sample`，与 `sample_card_*` 同路数）：外形与真件相同，
  概念绑定**完全相同**（一枚样本确实"是"那种宝石），但**独立模板** ——
  所以不混进"每色 7 枚"的账，销毁也不影响账。

由此产生一条**通用规则**（引擎与校验器都有镜像实现）：

> 样本与真件绑同一套概念 ⇒ "按概念点名"会同时命中两者。
> 凡是"要搬/要补一件真件"的判定（`transfer` 的 `what`、`start.set` 预置），
> 样本**不参与竞争**（`sample: true` 的模板排除，`ConceptCandidatesReal`）；
> 高亮/销毁**不排除** —— 它们本来就该能点到展示位上的样本。

不这么分的后果（都真发生过）：`start.set` 判"候选不唯一"→ 整条预置被静默跳过；
"盒里的黄金"被判成说不清是哪一件。

### `face` 的适用边界（血泪）

`Flipped` 是**逻辑朝向**，它只在"这件真的有另一面"时才有意义 —— 判据是**有没有背图**。

| 件 | 背图 | face 有意义？ |
|---|---|---|
| 市场真卡 `market_card_*` | 有 | ✅ 有（发牌 = 翻到正面） |
| 牌堆垫牌 `blank_card_*` | 无（它的"正面"就是卡背图） | ❌ |
| 样本卡 `sample_card_*` / `sample_back_*` | 无 | ❌ |
| 宝石 `gem` / 样本宝石 `gem_sample` | 无 | ❌ |
| 贵族 `noble` | 无（只有一面） | ❌ |

给 ❌ 那些件断 `face`，得到的是**指向不存在状态**的差异
