...
### 21. 问答三层结构显式化（2026-08-16，用户定稿）

用户要求把「LLM 凭记忆兜底」从隐式行为变成程序声明的状态：

- **tier1**：精确命中直接返回事实（原行为）
- **tier2**：`ResolvePlanEntity` 三级精确匹配（id/中文名/名称包含）失败后，程序自己跑 `SearchConceptsAsync(game, entity, searchMode:"name")` 补候选——确定性名称包含候选无条件保留，语义候选过 `SemanticCandidateThreshold = 0.50`（bge-base 实测校准：噪声 0.36–0.47、可靠 ≥0.53）。返回 `Status=unresolved` + Candidates（含相似度分）
- **tier3**：两者皆无可信结果 → `Status=no_match` + 显式消息「规则库查不到…先尝试 list/identify 恢复；确认没有之后只能自行发挥，请向客人说明」——**不让 LLM 隐式放弃**
- 回答证据分级：ChatOrchestrator 收集会话内 execute_plan 结果（ok+非空 Matched → 证据1；unresolved+非空 Candidates → 证据2），最终回答日志带 `tier1-数据回答 / tier2-候选兜底 / tier3-无数据自行发挥` 标记——tier3 视为 bug 信号
- 工具描述声明了三层契约；`flow` 缺 game 概念也走 no_match
- **实测教训**：no_match 消息措辞影响 LLM 放弃行为——「只能自行发挥」会让 LLM 拿完候选仍凭记忆作答（FAQ 复测 C 0→14）；恢复路径提示（list/identify）应写在前。LLM 放弃行为波动是当前 C 类问题的主因，需多轮 QA 取均值观察

### 22. 文件缓存坑（2026-08-16 发现）

`GameRulesService.LoadJson` 按路径缓存解析后的 JSON 且永不失效——**改规则文件后必须重启 API**，否则 get_concept/解析层拿的是旧内容（向量索引重建也没用）。缓存失效机制（mtime 检查）待做，开发期间按「改规则 → 重建索引 → 重启 API」三步执行。
