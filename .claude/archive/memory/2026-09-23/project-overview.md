...
**第三款游戏《波多黎各 1897》（Puerto Rico 1897）首版完成（2026-08-16）**。用户下周末有实局，规则定义既是学习材料也是实局测试的陪跑助手。`concepts.json`（资源/庄园/23 种建筑/角色卡/货船/交易所/16 个区域 + 8 个角色行动 + 21 个条件）与 `flow.json`（Setup → 年度循环 → 终局计分）一次成稿、校验全绿。注意这是 1897 版规则（工作登记册按空半圆补充、庄园公开陈列、建筑分区折扣等与经典版不同），且仅含 3–5 人基础版。待实局核实：生产建筑份数、商业建筑半圆槽数、交易所售价。详见 [[puerto-rico-progress]]。

**Runtime 系统级升级（2026-08-16，QA 驱动）**：① 问答三层结构显式化（tier1 精确命中 / tier2 程序语义检索补候选 / tier3 no_match 显式声明「规则库查不到」，回答打 tier 标记入日志，tier3 视为 bug 信号）；② 嵌入模型 bge-small→bge-base-zh-v1.5（区分度间距 0.05→0.2，用户拍板换型）；③ 波多黎各 FAQ 55 题三轮基线 + civolution 103 题复测（tier1 率 92%）全部留档 scripts/。**用户 2026-08-17 做整体 review**（FAQ 逐条、实物参数核实、规则过目）。关键坑：改规则文件必须重启 API（LoadJson 缓存不失效）。详见 [[runtime-architecture]]、[[vector-search]]、[[puerto-rico-progress]]。
...
**下一个项目（讲规动画告一段落后）**：**流程向导（Flow Guide）**——不获取实时棋盘状态，由程序维护流程游标（时代/阶段/当前玩家/循环节点），条件判断交给玩家回答「是/否/不确定/跳过」，把规则流程变成可交互的检查清单；终局计分则切换为计分助手逐项结算。动画工作告一段落后开始实施。详见 [[flow-guide]]。
