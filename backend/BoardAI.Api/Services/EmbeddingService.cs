...
public class EmbeddingService : IDisposable
{
    ...
    public EmbeddingService(string modelDir)
    {
        ...
        // 优先使用 CUDA GPU；如果当前环境没有可用 GPU，则自动回退 CPU。
        try
        {
            ...
            Console.Error.WriteLine("[Embedding] Using CUDA execution provider.");
        }
        catch (Exception ex)
        {
            Console.Error.WriteLine($"[Embedding] CUDA is unavailable, falling back to CPU: {ex.Message}");
        ...
        // 模型标签用于索引 payload——Python rebuild_index.py 也使用目录名作为 model_tag；
        // 换模型后标签变化，增量索引会据此强制重新 embedding。
        _modelTag = Path.GetFileName(Path.TrimEndingDirectorySeparator(modelDir));
    ...
    public string ModelTag => _modelTag;
