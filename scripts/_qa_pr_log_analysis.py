...
LOG = Path(r"D:\Temp\api.log")
RESULTS = Path(r"D:\workspace\board\scripts\_qa_pr_results.jsonl")

# 人工判分（基于上一轮 review）：这 4 题有瑕疵，其余正确（Q13 算可用）
BAD = {4, 27, 38, 43}
...
def main():
    ...
    # ---- 按请求头切分（同一问题文本保留最后一次出现 = 完整重跑）----
    header_re = re.compile(r"\[Chat\] game: puerto-rico, question: (.*?), count: \d+")
