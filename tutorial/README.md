...
2. **跨 cue 的链**：本 cue 的 `enter` 必须等于父 cue 的终态。
   父节点**默认是轨道里的上一条**（与运行时一致：顺序播放接着上一条的终态，
   跳转则从根按整轨顺序重放到本条之前）；`entry_from` 是**显式**写法，
   用于"时间上的上一条"不是"状态来源"时把话说明白 ——
   典型是有动画的 cue 之间夹着纯口播 cue（`setup.gems.001.1` 的状态来自
   `setup.cards.002.1`，中间隔着 13 号纯口播 cue）。不写也**会被查**（按默认父）；
   以前只在写了才查，于是没写的那些 cue 的进入状态根本没人比。
   只查自己那条，看不出**上一条**错没错。
